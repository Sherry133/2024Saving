...
(function ($) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		let headerElement = document.getElementById('header-placeholder');
		if (headerElement) {
			fetch('header.html')
				...
				.catch((error) =>
					console.error(
						'Error fetching or setting header content:',
						error
					)
				);
		} else {
			console.warn(
				"Element with id 'header-placeholder' not found. Skipping header fetch."
			);
		}
	});
